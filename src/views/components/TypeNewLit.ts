...
import { NewEntity } from '@/app/EntityType';
...
export class TypeNewLit extends LitElement {
  ...
  render() {
    const type = NodeTypeNew();
    return html`<div class="w-full">
      <button class="btn w-full flex cursor-pointer justify-center" @click="${() => this.opened = true}">${unsafeHTML(icon)}</button>
      <node-type-modal-lit .entity="${this.entity}" .type="${type}" .opened="${this.opened}" @close="${() => this.opened = false}"></node-type-modal-lit>
    </div>`;
  }
}
